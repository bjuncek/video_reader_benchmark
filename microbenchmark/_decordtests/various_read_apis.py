...
for i in range(10):
    for file in os.listdir("../videos"):
        ...
        times.append( timeit.timeit(f"get_decord_batch(\"{path}\")", setup=setup_decord, globals=globals(), number=args.n)/args.n)
        video.append(file)
        loaders.append("decord_VR_batch")
        num_frames.append(nframes)
        
        times.append( timeit.timeit(f"get_decord_loader(\"{path}\")", setup=setup_decord, globals=globals(), number=args.n)/args.n)
        video.append(file)
