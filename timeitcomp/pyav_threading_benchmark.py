...
for i in range(10):
    for path in videos:
        ...
        times.append(timeit.timeit(f"get_pyav_default(\"{path}\")", setup=setup_pyav, globals=globals(), number=NUBMER_TRIALS)/NUBMER_TRIALS)
        video.append(path)
        for t in ["AUTO", "SLICE", "FRAME"]:
            thread_type.append(t)
            video.append(path)
            times.append(timeit.timeit(f"get_pyav(\"{path}\", \"{t}\")", setup=setup_pyav, globals=globals(), number=NUBMER_TRIALS)/NUBMER_TRIALS)
